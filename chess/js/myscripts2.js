...
function getnames() {
  $.ajax({
    ...
    success: function (beta) {
      ...
      blackplayerrating = beta.substr(0, ajaxin);
      ajaxind = ajaxin + 1;
      while (beta[ajaxind] != " ") {
        ajaxind++;
      }
      blackplayername = beta.substr(ajaxin + 1, ajaxind - ajaxin);
      ajaxinde = ajaxind + 1;
      while (beta[ajaxinde] != " ") {
        ajaxinde++;
      }
      whiteplayerrating = beta.substr(ajaxind + 1, ajaxinde - ajaxind);
      whiteplayername = beta.substr(ajaxinde + 1, beta.length - (ajaxinde + 1));
      console.log(whiteplayerrating);
      $("#blackplayer").text(
        "Playing as black:" + blackplayername + " Rating:" + blackplayerrating
      );
      $("#whiteplayer").text(
        "Playing as white:" + whiteplayername + " Rating:" + whiteplayerrating
      );
    },
  });
}
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (game.moves.length > 0) {
        piece = $("#p" + game.moves[1] + game.moves[2])
          .children()
          .attr("id");
...
function whitequeenmove(attackedpiece, position2, pawnid) {
  ...
  if (
    checkifplayerisincheck(
      $("#" + window.activeplayer + "king")
        .parent()
        .attr("id")
    )
  ) {
    ...
    $("#submitmove").html("black move");
    logmove(
      "p" + window.pos1.substr(1, 2) + "W" + piece[0] + position2.substr(1, 2)
    );
...
function blackqueenmove(attackedpiece, position2, pawnid) {
  ...
  if (
    ...
    $("#submitmove").html("white move");
    logmove(
      "p" + window.pos1.substr(1, 2) + "E" + piece[0] + position2.substr(1, 2)
    );
    if (states.wcheck || states.bcheck) {
      if (checkformate()) {
        updateratings("b", 1);
...
function kingstale(rowk, colk) {
  ...
  for (cou = -1; cou < 2; cou++) {
    ...
    if (
      $(
        "#p" + (parseInt(rowk) + cou) + (parseInt(colk) + 1).toString()
      ).children().length == 0
    ) {
      if (
        parseInt(rowk) + cou < 9 &&
...
function checkformate() {
  mate = true;
  kingposition = $("#" + window.activeplayer + "king")
    .parent()
    .attr("id");
...
function pawnstale() {
  for (i = 1; i < 9; i++) {
    if (
      $("#" + window.activeplayer + "pawn" + i).length > 0 &&
      $("#" + window.activeplayer + "pawn" + i)
        .parent()
        .attr("id") != "yeet"
    ) {
      row1 = $("#" + window.activeplayer + "pawn" + i)
        .parent()
        .attr("id")
        .charAt(1);
      col1 = $("#" + window.activeplayer + "pawn" + i)
        .parent()
        .attr("id")
        .charAt(2);
      ...
      if ($(str2).children() > 0) {
        if (
          window.activeplayer == "black" &&
          $(str2).children().attr("id").charAt(0) == "w"
        ) {
          return false;
        }
      }
      if ($(str3).children() > 0) {
        if (
          window.activeplayer == "black" &&
          $(str3).children().attr("id").charAt(0) == "w"
        ) {
          return false;
        }
      } else if (
        window.activeplayer == "white" &&
        $(str4).children().length == 0
      ) {
        return false;
      }
      if ($(str5).children() > 0) {
        if (
          window.activeplayer == "white" &&
          $(str5).children().attr("id").charAt(0) == "w"
        ) {
          return false;
        }
      }
      if ($(str6).children() > 0) {
        if (
          window.activeplayer == "white" &&
          $(str6).children().attr("id").charAt(0) == "w"
        ) {
          return false;
        }
      }
...
function bishopstale() {
  for (j = 1; j < 11; j++) {
    if (
      ...
      row1 = $("#" + window.activeplayer + "bishop" + j)
        .parent()
        .attr("id")
        .charAt(1);
      col1 = $("#" + window.activeplayer + "bishop" + j)
        .parent()
        .attr("id")
        .charAt(2);
      if (
        $("#p" + (parseInt(row1) + 1) + (parseInt(col1) + 1).toString())
          .length > 0
      ) {
        if (
          $(
          ...
          $("#p" + (parseInt(row1) + 1) + (parseInt(col1) + 1).toString())
            .children()
            .attr("id")
            .charAt(0) != window.activeplayer.charAt(0)
        ) {
          return false;
        }
      }
...
function rookstale() {
  for (k = 1; k < 11; k++) {
    if (
      ...
      row1 = $("#" + window.activeplayer + "rook" + k)
        .parent()
        .attr("id")
        .charAt(1);
      col1 = $("#" + window.activeplayer + "rook" + k)
        .parent()
        .attr("id")
        .charAt(2);
      if (
        $("#p" + parseInt(row1) + (parseInt(col1) + 1).toString()).length > 0
      ) {
        if (
          $("#p" + parseInt(row1) + (parseInt(col1) + 1).toString()).children()
            .length == 0
        ) {
          return false;
        } else if (
          $("#p" + parseInt(row1) + (parseInt(col1) + 1).toString())
            .children()
            .attr("id")
            .charAt(0) != window.activeplayer.charAt(0)
        ) {
...
function queenstale() {
  for (q = 1; q < 11; q++) {
    if (
      ...
      row1 = $("#" + window.activeplayer + "queen" + q)
        .parent()
        .attr("id")
        .charAt(1);
      col1 = $("#" + window.activeplayer + "queen" + q)
        .parent()
        .attr("id")
        .charAt(2);
      if (
...
function knightstale() {
  for (l = 1; l < 11; l++) {
    if (
      $("#" + window.activeplayer + "knight" + l).length > 0 &&
      $("#" + window.activeplayer + "knight" + l)
        .parent()
        .attr("id") != undefined
    ) {
      row1 = $("#" + window.activeplayer + "knight" + l)
        .parent()
        .attr("id")
        .charAt(1);
      col1 = $("#" + window.activeplayer + "knight" + l)
        .parent()
        .attr("id")
        .charAt(2);
      if (
        $("#p" + ((parseInt(row1) + 2).toString() + (parseInt(col1) - 1)))
          .length > 0
      ) {
        if (
          $(
      ...
      if (
        $("#p" + ((parseInt(row1) - 1).toString() + (parseInt(col1) + 2)))
          .length > 0
      ) {
        if (
          $(
...
function verifymoveblack(attackedpiece, position2) {
  ...
  if (
    ...
    logmove(
      window.piece[5] +
        window.pos1.substr(1, 2) +
        window.piece[5] +
        position2.substr(1, 2) +
        "XX"
    );
    if (states.wcheck || states.bcheck) {
      if (checkformate()) {
...
function moveknight(position2) {
  if (position2 != undefined) {
    ...
    if (
      (row2 == row1 - 2 || row1 == row2 - 2) &&
      (col1 == col2 - 1 || col2 == col1 - 1) &&
      window.activeplayer == "black"
    ) {
      verifymoveblack(
        $("#" + position2)
      ...
      (row2 == row1 - 1 || row1 == row2 - 1) &&
      (col1 == col2 - 2 || col2 == col1 - 2) &&
      window.activeplayer == "white"
    ) {
      verifymovewhite(
        $("#" + position2)
...
function movepawn(position2) {
  if (position2 != undefined) {
    ...
    if (
      ...
      if (
        $("#p" + (row1 - 1) + col1).children().length == 0 &&
        $("#p" + row2 + col2).children().length == 0 &&
        window.activeplayer == "white"
      ) {
        verifymovewhite(
...
function kingmovecheck(row1, col1, row2, col2) {
  if (
    (row2 == row1 || row1 == row2 - 1 || row2 == row1 - 1) &&
    (col2 == col1 || col1 == col2 - 1 || col2 == col1 - 1)
  ) {
    return true;
  } else {
    return false;
...
function checkforcheck(position) {
  ...
  if (window.piece.indexOf("knight") != -1) {
    ...
    if (
      bishopmovecheck(row1, col1, row2, col2) ||
      rookmovecheck(row1, col1, row2, col2)
    ) {
      check = true;
    }
...
$("img").click(function (event) {
  event.stopPropagation();
  if (
    $("img").parent().css("background-color") != "blue" &&
    event.target.id.indexOf(window.activeplayer) != -1 &&
    event.target.id[0] == game.activeuser[0]
  ) {
    $("img").parent().css("background-color", "");
    $("#" + event.target.id)
      .parent()
      .css("background-color", "blue");
    window.pos1 = $("#" + event.target.id)
      .parent()
      .attr("id");
